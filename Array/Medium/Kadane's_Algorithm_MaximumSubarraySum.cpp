#include<bits/stdc++.h>
using namespace std;


int bruteforce(int arr[], int n) {
    //  O(N3)
    // SC: O(1)
    int maxi = INT_MIN; // maximum sum

    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            // subarray = arr[i.....j]
            int sum = 0;

            //add all the elements of subarray:
            for (int k = i; k <= j; k++) {
                sum += arr[k];
            }

            maxi = max(maxi, sum);
        }
    }

    return maxi;
}

int better(int arr[], int n) {
    // O(N2)
    int maxi = INT_MIN; // maximum sum

    for (int i = 0; i < n; i++) {
        int sum = 0;
        for (int j = i; j < n; j++) {
            // current subarray = arr[i.....j]

            //add the current element arr[j]
            // to the sum i.e. sum of arr[i...j-1]
            sum += arr[j];

            maxi = max(maxi, sum); // getting the maximum
        }
    }

    return maxi;
}


// Kadane's Algorithm
long long optimal(int arr[], int n) {
    // O(N)

    long long maxi = LONG_MIN; // maximum sum
    long long sum = 0;

    for (int i = 0; i < n; i++) {

        sum += arr[i];

        if (sum > maxi) {
            maxi = sum;
        }

        // If sum < 0: discard the sum calculated
        if (sum < 0) {
            sum = 0;
        }
    }

    // To consider the sum of the empty subarray
    // uncomment the following check:

    //if (maxi < 0) maxi = 0;

    return maxi;
}



long long followUpQue(int arr[], int n) {
    // O(n)

    long long maxi = LONG_MIN; // maximum sum
    long long sum = 0;

    int start = 0;
    int ansStart = -1, ansEnd = -1;
    for (int i = 0; i < n; i++) {

        if (sum == 0) start = i; // starting index

        sum += arr[i];

        if (sum > maxi) {
            maxi = sum;

            ansStart = start;
            ansEnd = i;
        }

        // If sum < 0: discard the sum calculated
        if (sum < 0) {
            sum = 0;
        }
    }

    //printing the subarray:
    cout << "The subarray is: [";
    for (int i = ansStart; i <= ansEnd; i++) {
        cout << arr[i] << " ";
    }
    cout << "]n";

    // To consider the sum of the empty subarray
    // uncomment the following check:

    //if (maxi < 0) maxi = 0;

    return maxi;
}

int main()
{
    int arr[] = { -2, 1, -3, 4, -1, 2, 1, -5, 4};
    int n = sizeof(arr) / sizeof(arr[0]);
    int maxSum = bruteforce(arr, n);
    cout << "The maximum subarray sum is: " << maxSum << endl;


    int arr1[] = { -2, 1, -3, 4, -1, 2, 1, -5, 4};
    int n1 = sizeof(arr1) / sizeof(arr1[0]);
    int maxSum1 = better(arr1, n1);
    cout << "The maximum subarray sum is: " << maxSum1 << endl;


    int arr2[] = { -2, 1, -3, 4, -1, 2, 1, -5, 4};
    int n2 = sizeof(arr2) / sizeof(arr2[0]);
    long long maxSum2 = optimal(arr2, n2);
    cout << "The maximum subarray sum is: " << maxSum2 << endl;



    int arr3[] = { -2, 1, -3, 4, -1, 2, 1, -5, 4};
    int n3 = sizeof(arr3) / sizeof(arr3[0]);
    long long maxSum3 = followUpQue(arr3, n3);
    cout << "The maximum subarray sum is: " << maxSum3 << endl;

    return 0;
}